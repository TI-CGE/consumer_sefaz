...
public class DespesaConvenioPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE DESPESA CONVÊNIO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        List<DespesaConvenioDTO> resultadoConsolidado = new ArrayList<>();
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("DESPESA_CONVENIO", "Consumindo Despesa Convênio ano " + ano + " UG " + cdUnidadeGestora);
        for (int mes = 1; mes <= 12; mes++) {
            simpleLogger.consumptionProgress("DESPESA_CONVENIO", "Processando meses ano " + ano, mes, 12, "Mês: " + mes);
            ...
            try {
                ...
                dto.setNuAnoLancamentoFiltro(ano);
                dto.setNuMesLancamentoFiltro(mes);
                List<DespesaConvenioDTO> resultadoMes = consumoApiService.consumirPersistir(dto, ugFiltro);
                if (resultadoMes != null && !resultadoMes.isEmpty()) {
                    resultadoConsolidado.addAll(resultadoMes);
                    logger.info("Ano " + ano + " Mês " + mes + ": " + resultadoMes.size() + " registros processados");
                } else {
    ...
    public List<DespesaConvenioDTO> consumirAnoEMes(int ano, int mes, String cdUnidadeGestora) {
        if (cdUnidadeGestora == null || cdUnidadeGestora.trim().isEmpty()) {
            throw new IllegalArgumentException("cdUnidadeGestora é obrigatório");
        }
        if (ano < ANO_MIN || ano > ANO_MAX) {
        ...
        logger.info("=== CONSUMINDO DESPESA CONVÊNIO - ANO " + ano + " MÊS " + mes + " UG " + cdUnidadeGestora + " ===");
        DespesaConvenioDTO dto = new DespesaConvenioDTO();
        dto.setNuAnoLancamentoFiltro(ano);
        dto.setNuMesLancamentoFiltro(mes);
        List<DespesaConvenioDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
