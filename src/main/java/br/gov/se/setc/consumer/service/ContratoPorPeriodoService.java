...
public class ContratoPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE CONTRATO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("CONTRATO", "Consumindo Contrato ano " + ano + " UG " + cdUnidadeGestora);
        ContratoDTO dto = new ContratoDTO();
        dto.setDtAnoExercicioFiltro(ano);
        List<ContratoDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
        long executionTimeMs = System.currentTimeMillis() - startTime;
        int total = resultado != null ? resultado.size() : 0;
        simpleLogger.consumptionEnd("CONTRATO", total + " registros processados (ano " + ano + " UG " + cdUnidadeGestora + ")", executionTimeMs);
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", total);
        resumo.put("ano", ano);
