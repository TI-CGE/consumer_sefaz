...
@Service
public class LiquidacaoPorPeriodoService {
    private static final int ANO_MIN = 2000;
    private static final int ANO_MAX = 2030;
    private static final Logger logger = Logger.getLogger(LiquidacaoPorPeriodoService.class.getName());
    private final ConsumoApiService<LiquidacaoDTO> consumoApiService;
    @Autowired
    private SimpleLogger simpleLogger;

    public LiquidacaoPorPeriodoService(
    ...
    public Map<String, Object> consumirAnoInteiro(int ano) {
        ...
        logger.info("=== INICIANDO CONSUMO DE LIQUIDAÇÃO - ANO INTEIRO " + ano + " ===");
        List<LiquidacaoDTO> resultadoConsolidado = new ArrayList<>();
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("LIQUIDACAO", "Consumindo Liquidação para todos os 12 meses do ano " + ano);
        for (int mes = 1; mes <= 12; mes++) {
            simpleLogger.consumptionProgress("LIQUIDACAO", "Processando meses ano " + ano, mes, 12, "Mês: " + mes);
            ...
            try {
                ...
                List<LiquidacaoDTO> resultadoMes = consumoApiService.consumirPersistir(dto);
                ...
                throw new RuntimeException("Erro ao consumir Liquidação para ano " + ano + " mês " + mes + ": " + e.getMessage(), e);
            }
        }
        long executionTimeMs = System.currentTimeMillis() - startTime;
        simpleLogger.consumptionEnd("LIQUIDACAO",
                resultadoConsolidado.size() + " registros processados (ano " + ano + ")",
                executionTimeMs);
        logger.info("=== CONSUMO LIQUIDAÇÃO ANO INTEIRO " + ano + " CONCLUÍDO === Total: " + resultadoConsolidado.size() + " registros");
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE LIQUIDAÇÃO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        List<LiquidacaoDTO> resultadoConsolidado = new ArrayList<>();
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("LIQUIDACAO", "Consumindo Liquidação ano " + ano + " UG " + cdUnidadeGestora);
        for (int mes = 1; mes <= 12; mes++) {
            simpleLogger.consumptionProgress("LIQUIDACAO", "Processando meses ano " + ano, mes, 12, "Mês: " + mes);
            ...
            try {
                ...
                dto.setDtAnoExercicioCTBFiltro(ano);
                dto.setNuMesFiltro(mes);
                List<LiquidacaoDTO> resultadoMes = consumoApiService.consumirPersistir(dto, ugFiltro);
                if (resultadoMes != null && !resultadoMes.isEmpty()) {
                    resultadoConsolidado.addAll(resultadoMes);
                    logger.info("Ano " + ano + " Mês " + mes + ": " + resultadoMes.size() + " registros processados");
                } else {
        ...
        simpleLogger.consumptionEnd("LIQUIDACAO",
                ...
                executionTimeMs);
        logger.info("=== CONSUMO LIQUIDAÇÃO ANO INTEIRO " + ano + " UG " + cdUnidadeGestora + " CONCLUÍDO === Total: " + resultadoConsolidado.size() + " registros");
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", resultadoConsolidado.size());
        resumo.put("ano", ano);
    ...
    public List<LiquidacaoDTO> consumirAnoEMes(int ano, int mes) {
        if (ano < ANO_MIN || ano > ANO_MAX) {
            throw new IllegalArgumentException("Ano deve estar entre " + ANO_MIN + " e " + ANO_MAX);
        }
        if (mes < 1 || mes > 12) {
        ...
        logger.info("=== CONSUMINDO LIQUIDAÇÃO - ANO " + ano + " MÊS " + mes + " ===");
        long startTime = System.currentTimeMillis();
        LiquidacaoDTO dto = new LiquidacaoDTO();
        dto.setDtAnoExercicioCTBFiltro(ano);
        dto.setNuMesFiltro(mes);
        List<LiquidacaoDTO> resultado = consumoApiService.consumirPersistir(dto);
        long executionTimeMs = System.currentTimeMillis() - startTime;
    ...
    public List<LiquidacaoDTO> consumirAnoEMes(int ano, int mes, String cdUnidadeGestora) {
        if (cdUnidadeGestora == null || cdUnidadeGestora.trim().isEmpty()) {
            throw new IllegalArgumentException("cdUnidadeGestora é obrigatório");
        }
        if (ano < ANO_MIN || ano > ANO_MAX) {
        ...
        logger.info("=== CONSUMINDO LIQUIDAÇÃO - ANO " + ano + " MÊS " + mes + " UG " + cdUnidadeGestora + " ===");
        long startTime = System.currentTimeMillis();
        LiquidacaoDTO dto = new LiquidacaoDTO();
        dto.setDtAnoExercicioCTBFiltro(ano);
        dto.setNuMesFiltro(mes);
        List<LiquidacaoDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
        long executionTimeMs = System.currentTimeMillis() - startTime;
