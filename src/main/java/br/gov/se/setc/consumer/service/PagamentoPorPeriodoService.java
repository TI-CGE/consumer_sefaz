...
public class PagamentoPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE PAGAMENTO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        List<PagamentoDTO> resultadoConsolidado = new ArrayList<>();
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("PAGAMENTO", "Consumindo Pagamento ano " + ano + " UG " + cdUnidadeGestora);
        for (int mes = 1; mes <= 12; mes++) {
            simpleLogger.consumptionProgress("PAGAMENTO", "Processando meses ano " + ano, mes, 12, "Mês: " + mes);
            ...
            try {
                ...
                dto.setDtAnoExercicioCTBFiltro(ano);
                dto.setNuMesFiltro(mes);
                List<PagamentoDTO> resultadoMes = consumoApiService.consumirPersistir(dto, ugFiltro);
                if (resultadoMes != null && !resultadoMes.isEmpty()) {
                    resultadoConsolidado.addAll(resultadoMes);
                    logger.info("Ano " + ano + " Mês " + mes + ": " + resultadoMes.size() + " registros processados");
                } else {
        ...
        simpleLogger.consumptionEnd("PAGAMENTO",
                ...
                executionTimeMs);
        logger.info("=== CONSUMO PAGAMENTO ANO INTEIRO " + ano + " UG " + cdUnidadeGestora + " CONCLUÍDO === Total: " + resultadoConsolidado.size() + " registros");
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", resultadoConsolidado.size());
        resumo.put("ano", ano);
    ...
    public List<PagamentoDTO> consumirAnoEMes(int ano, int mes, String cdUnidadeGestora) {
        if (cdUnidadeGestora == null || cdUnidadeGestora.trim().isEmpty()) {
            throw new IllegalArgumentException("cdUnidadeGestora é obrigatório");
        }
        if (ano < ANO_MIN || ano > ANO_MAX) {
        ...
        logger.info("=== CONSUMINDO PAGAMENTO - ANO " + ano + " MÊS " + mes + " UG " + cdUnidadeGestora + " ===");
        long startTime = System.currentTimeMillis();
        PagamentoDTO dto = new PagamentoDTO();
        dto.setDtAnoExercicioCTBFiltro(ano);
        dto.setNuMesFiltro(mes);
        List<PagamentoDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
        long executionTimeMs = System.currentTimeMillis() - startTime;
