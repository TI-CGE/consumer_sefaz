...
public class ConsultaGerencialPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE CONSULTA GERENCIAL - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        List<ConsultaGerencialDTO> resultadoConsolidado = new ArrayList<>();
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("CONSULTA_GERENCIAL", "Consumindo Consulta Gerencial ano " + ano + " UG " + cdUnidadeGestora);
        for (int mes = 1; mes <= 12; mes++) {
            simpleLogger.consumptionProgress("CONSULTA_GERENCIAL", "Processando meses ano " + ano, mes, 12, "Mês: " + mes);
        ...
        simpleLogger.consumptionEnd("CONSULTA_GERENCIAL",
                ...
                executionTimeMs);
        logger.info("=== CONSUMO CONSULTA GERENCIAL ANO INTEIRO " + ano + " UG " + cdUnidadeGestora + " CONCLUÍDO === Total: " + resultadoConsolidado.size() + " registros");
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", resultadoConsolidado.size());
        resumo.put("ano", ano);
    ...
    public List<ConsultaGerencialDTO> consumirAnoEMes(int ano, int mes, String cdUnidadeGestora) {
        if (cdUnidadeGestora == null || cdUnidadeGestora.trim().isEmpty()) {
            throw new IllegalArgumentException("cdUnidadeGestora é obrigatório");
        }
        if (ano < ANO_MIN || ano > ANO_MAX) {
        ...
        logger.info("=== CONSUMINDO CONSULTA GERENCIAL - ANO " + ano + " MÊS " + mes + " UG " + cdUnidadeGestora + " ===");
        ConsultaGerencialDTO dto = new ConsultaGerencialDTO();
        dto.setDtAnoExercicioCTBFiltro(ano);
        dto.setNuMesFiltro(mes);
        List<ConsultaGerencialDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
