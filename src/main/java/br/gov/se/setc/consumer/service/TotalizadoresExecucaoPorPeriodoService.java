...
public class TotalizadoresExecucaoPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE TOTALIZADORES EXECUÇÃO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("TOTALIZADORES_EXECUCAO", "Consumindo Totalizadores Execução ano " + ano + " UG " + cdUnidadeGestora);
        TotalizadoresExecucaoDTO dto = new TotalizadoresExecucaoDTO();
        dto.setDtAnoExercicioCTBFiltro(ano);
        List<TotalizadoresExecucaoDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
        long executionTimeMs = System.currentTimeMillis() - startTime;
        int total = resultado != null ? resultado.size() : 0;
        simpleLogger.consumptionEnd("TOTALIZADORES_EXECUCAO", total + " registros processados (ano " + ano + " UG " + cdUnidadeGestora + ")", executionTimeMs);
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", total);
        resumo.put("ano", ano);
