...
public class ContratoEmpenhoPorPeriodoService {
    ...
    public Map<String, Object> consumirAnoInteiro(int ano, String cdUnidadeGestora) {
        ...
        logger.info("=== INICIANDO CONSUMO DE CONTRATO EMPENHO - ANO " + ano + " UG " + cdUnidadeGestora + " ===");
        long startTime = System.currentTimeMillis();
        simpleLogger.consumptionStart("CONTRATO_EMPENHO", "Consumindo Contrato Empenho ano " + ano + " UG " + cdUnidadeGestora);
        ContratoEmpenhoDTO dto = new ContratoEmpenhoDTO();
        dto.setDtAnoExercicioFiltro(ano);
        List<ContratoEmpenhoDTO> resultado = consumoApiService.consumirPersistir(dto, Collections.singletonList(cdUnidadeGestora.trim()));
        long executionTimeMs = System.currentTimeMillis() - startTime;
        int total = resultado != null ? resultado.size() : 0;
        simpleLogger.consumptionEnd("CONTRATO_EMPENHO", total + " registros processados (ano " + ano + " UG " + cdUnidadeGestora + ")", executionTimeMs);
        Map<String, Object> resumo = new HashMap<>();
        resumo.put("status", "SUCCESS");
        resumo.put("recordsProcessed", total);
        resumo.put("ano", ano);
